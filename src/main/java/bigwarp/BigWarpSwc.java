...
public class BigWarpSwc< T > extends BigWarp<T>
{
	...
	public void loadSwcDialog()
	{
		...
		gd.addCheckbox("moving space", false);
		gd.showDialog();

		if ( gd.wasCanceled() )
			return;

		final String swcPath = gd.getNextString();
		final boolean movingSpace = gd.getNextBoolean();
		loadSwc( swcPath, movingSpace, swcTransform );
	}
